import {debounce, distinctUntilChanged, latest} from '../src';
import {sleep} from "./utils";

describe('combined', () => {
    test('debounce with distinctUntilChanged', async () => {
        const results: number[] = [];
        const applyDebounceDistinctUntilChanged = debounce(distinctUntilChanged((i: number) => results.push(i)), 10);
    ...
    test('distinctUntilChanged with latest', async () => {
        ...
        const applyDistinctUntilChangedLatest = distinctUntilChanged(latest(generator));
    ...
    test('latest with debounce', async () => {
        ...
        const applyLatestDebounce = latest(debounce(generator, 0));
    ...
    test('distinctUntilLatest with latest and debounce', async () => {
        ...
        const allCombined = distinctUntilChanged(latest(debounce(generator, 0)));
